...
const { MarketplaceContractSignatureETH, ACTIVE_NETWORK_ETH } = require(".");
...
export default class LazyMinter {
  ...
  async createVoucher(
    ...
    try {
      const nftAddress = MarketplaceContractSignatureETH;
      const order = {
        seller,
        nftAddress,
        price,
      ...
      const types = {
        Order: [
          ...
          { name: "isNewColl", type: "bool" },
          { name: "lockedContent", type: "string" },
        ],
      };
  ...
  async _signingDomain() {
    ...
    // const chainId = "0x" + ACTIVE_NETWORK_ETH.toString(16);
    const chainId = ACTIVE_NETWORK_ETH;

    this._domain = {
      name: SIGNING_DOMAIN_NAME,
      version: SIGNING_DOMAIN_VERSION,
      verifyingContract: MarketplaceContractSignatureETH,
      chainId,
    };
    return this._domain;
  }
